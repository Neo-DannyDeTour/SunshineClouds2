...
func _process(delta : float) -> void:
	if clouds_resource != null:
		clouds_resource.current_time = wrap(clouds_resource.current_time
			+ delta * clouds_resource.dither_speed, 0.0, clouds_resource.dither_speed * 64.0)
		if update_continuously:
			...
			small_clouds_pos += ((wind_direction * small_structures_wind_speed)
				+ (Vector3.UP * abs(small_structures_wind_speed))) * delta
			...
			if (tracked_directional_lights.size()
				* 2.0 != clouds_resource.directional_lights_data.size() \
			...
			for i in range(tracked_directional_lights.size()):
				if tracked_directional_lights[i] == null:
					continue
				if direction_light_data_changed(tracked_directional_lights[i],
					tracked_directional_light_shadow_steps[i],
						clouds_resource.directional_lights_data[i * 2],
							clouds_resource.directional_lights_data[i * 2 + 1]):
					retrieve_texture_data()
					return
			for i in range(tracked_point_lights.size()):
				if tracked_point_lights[i] == null:
					continue
				if point_light_data_changed(tracked_point_lights[i],
					clouds_resource.point_lights_data[i * 2],
						clouds_resource.point_lights_data[i * 2 + 1]):
					retrieve_texture_data()
					return
			for i in range(tracked_point_effectors.size()):
				if tracked_point_effectors[i] == null:
					continue
				if point_effector_data_changed(tracked_point_effectors[i],
					clouds_resource.point_effector_data[i * 2],
						clouds_resource.point_effector_data[i * 2 + 1]):
					retrieve_texture_data()
					return
	else:
		update_continuously = false
...
func clouds_res_removed() -> void:
	if clouds_resource && is_inside_tree():
		...
		if env && env.compositor != null:
			var effects: Variant = env.compositor.compositor_effects
...
func recursively_find_env(this_node: Node) -> WorldEnvironment:
	for child in this_node.get_children():
		if child is WorldEnvironment:
			return child
		var result: Variant = recursively_find_env(child)
		if (result):
			return result
	return null
func retrieve_texture_data() -> void:
	...
	if clouds_resource != null:
		...
		if tracked_directional_light_shadow_steps.size() < tracked_directional_lights.size():
			...
			for i in range(tracked_directional_lights.size()
				- tracked_directional_light_shadow_steps.size()):
				tracked_directional_light_shadow_steps.append(12)
		for i in range(tracked_directional_lights.size()):
			if tracked_directional_lights[i] != null:
				var light: Variant = tracked_directional_lights[i]
				var look_dir: Variant = light.global_transform.basis.z.normalized()
				clouds_resource.directional_lights_data.append(Vector4(look_dir.x,
					look_dir.y, look_dir.z,
					tracked_directional_light_shadow_steps[i]))
				clouds_resource.directional_lights_data.append(Vector4(light.light_color.r,
					light.light_color.g, light.light_color.b,

						round(light.light_color.a * light.light_energy
							* directional_light_power_multiplier * 10.0) / 10.0))
		for i in range(tracked_point_lights.size()):
			if tracked_point_lights[i] != null:
				var light: Variant = tracked_point_lights[i]
				var light_pos: Variant = light.global_position
				clouds_resource.point_lights_data.append(Vector4(light_pos.x,
					light_pos.y, light_pos.z,
					light.omni_range))
				clouds_resource.point_lights_data.append(Vector4(light.light_color.r,
					light.light_color.g,
					light.light_color.b,
						round(light.light_color.a * light.light_energy
							* point_light_power_multiplier * 10.0) / 10.0))
		for i in range(tracked_point_effectors.size()):
			if tracked_point_effectors[i] != null:
				var node: Variant = tracked_point_effectors[i]
				var node_pos: Variant = node.global_position
				clouds_resource.point_effector_data.append(Vector4(node_pos.x,
					node_pos.y, node_pos.z,
					node.radius))
...
func wrap_vector(target: Vector3, domain_size: float) -> Vector3:
	...
	if target.z > domain_size:
		...
		target.z += domain_size * 2.0
	return target
func direction_light_data_changed(light : DirectionalLight3D, shadow_count : int,
	dir_data : Vector4, color_data : Vector4) -> bool:
	return light.global_transform.basis.z.x != dir_data.x \
		or light.global_transform.basis.z.y != dir_data.y \
		or light.global_transform.basis.z.z != dir_data.z \
		or float(shadow_count) != dir_data.w \
		or light.light_color.r != color_data.x \
		or light.light_color.g != color_data.y \
		or light.light_color.b != color_data.z \
		or round(light.light_color.a * light.light_energy * \
			directional_light_power_multiplier * 10.0) / 10.0 != round(color_data.w * 10.0) / 10.0
func point_light_data_changed(light : OmniLight3D, dir_data : Vector4,
	color_data : Vector4) -> bool:
	return light.global_position.x != dir_data.x \
		or light.global_position.y != dir_data.y \
		or light.global_position.z != dir_data.z \
		or light.omni_range != dir_data.w \
		or light.light_color.r != color_data.x \
		or light.light_color.g != color_data.y \
		or light.light_color.b != color_data.z \
		or round(light.light_color.a * light.light_energy * \
			point_light_power_multiplier * 10.0) / 10.0 != round(color_data.w * 10.0) / 10.0
# gdlint: disable=max-line-length
func point_effector_data_changed(node : SunshineCloudsEffector, dir_data : Vector4, color_data : Vector4) -> bool:
	return node.global_position.x != dir_data.x \
		or node.global_position.y != dir_data.y \
		or node.global_position.z != dir_data.z \
		or node.radius != dir_data.w \
		or node.power != color_data.x